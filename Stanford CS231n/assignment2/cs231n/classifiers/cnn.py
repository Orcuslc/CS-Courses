...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        stride_conv = 1
        pad = (filter_size-1)//2
        shape_after_conv = (num_filters, 1+int((H+2*pad-filter_size)/stride_conv), 1+int((W+2*pad-filter_size)/stride_conv))
        
        stride_pool = 2
        ph = pw = 2
        shape_after_pool = (num_filters, 1+int((shape_after_conv[1]-ph)/stride_pool), 1+int((shape_after_conv[2]-pw)/stride_pool))
        
        W2 = np.random.normal(loc = 0.0, scale = weight_scale, size = (np.prod(shape_after_pool), hidden_dim))
