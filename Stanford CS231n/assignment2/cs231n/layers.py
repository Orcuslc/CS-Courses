...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = {'mu':mu, 'sigma':sigma, 'x':x, 'out':out, 'eps':eps, 'x_hat': x_hat, 'gamma':gamma, 'beta':beta}
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    dbeta = dout.sum(axis = 0) ## sum of cols
    dgamma = (x_hat*dout).sum(axis = 0)
    
    std = np.sqrt(sigma+eps)
    dx_hat = gamma*dout
    dsigma = (-0.5*(x-mu)*std**-3*dx_hat).sum(axis = 0)
    dmu = (-1/std*dx_hat).sum(axis = 0) + dsigma*(-2/N*(x-mu).sum())
    
    dx = dx_hat*1/std + dsigma*2/N*(x-mu) + dmu/N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = 1/std*(dx_hat-np.mean(dx_hat, axis = 0) - np.mean(x_hat*dx_hat, axis = 0)*x_hat)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N):
        for j in range(F):
            for k in range(H_):
                for l in range(W_):
                    out[i, j, k, l] = np.sum(x_pad[i, :, k*stride:k*stride+HH, l*stride:l*stride+WW]\
                                            *w[j, :, :, :]) + b[j]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(F):
        for j in range(C):
            for k in range(HH):
                for l in range(WW):
                    dw[i, j, k, l] = np.sum(x_pad[:, j, k:k+stride*H_:stride, l:l+stride*W_:stride]*dout[:, i, :, :])
    for i in range(N):
        for j in range(H):
            for k in range(W):
                for l in range(F):
                    for u in range(H_):
                        for v in range(W_):
                            ...
                            xloc = j+pad-u*stride
                            yloc = k+pad-v*stride
                            if 0 <= xloc < HH and 0 <= yloc < WW:
                                mask[:, xloc, yloc] = 1
                            dx[i, :, j, k] += np.sum(mask*w[l, :, :, :]*dout[i, l, u, v], axis = (1, 2))
    for i in range(F):
        db[i] = np.sum(dout[:, i, :, :])
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    H_ = int((H-ph)/stride+1)
    W_ = int((W-pw)/stride+1)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for k in range(H_):
                for l in range(W_):
                    local = x[i, j, k*stride:k*stride+ph, l*stride:l*stride+pw]
                    mask = local == np.max(local)
                    dx[i, j, k*stride:k*stride+ph, l*stride:l*stride+pw] += dout[i, j, k, l]*mask
