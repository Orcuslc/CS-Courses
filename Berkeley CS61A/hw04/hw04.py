...
def with_totals(m):
    ...
    left = sides(m)[0]
    right = sides(m)[1]
    tmp = mobile(with_totals(end(left)), with_totals(end(right)))
    tmp[0] = total_weight(m)
    return tmp
...
class interval:
    ...
    def __add__(self, I):
        return self.make_interval(self._low+I.low(), self._high+I.high())
    def __sub__(self, I):
        return self.make_interval(self._low-I.high(), self._high-I.low())
    def __mul__(self, I):
        cases = [self._low*I.low(), self._low*I.high(), self._high*I.low(), self._high*I.high()]
        return self.make_interval(min(cases), max(cases))
    ...
    def __neg__(self):
        cases = [-1*self._low, -1*self._high]
        return self.make_interval(min(cases), max(cases))
