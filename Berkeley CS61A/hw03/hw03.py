...
def pingpong(n):
    ...
    #       flag *= -1
    # return i
    def get_sum(n, k, flag):
        if k == n+1:
            return 0
        if has_seven(k) or (k % 7 == 0):
            return flag + get_sum(n, k+1, -flag)
        else:
            return flag + get_sum(n, k+1, flag)
...
def change(lst, s, t):
    ...
    if tmp == s:
        tmp = t
    return link(tmp, change(rest(lst), s, t))
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return flatten([[deck[i], deck[len(deck)//2 + i]] for i in range(len(deck)//2)])
